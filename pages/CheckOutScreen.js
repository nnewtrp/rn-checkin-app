...
export default class CheckOutScreen extends React.Component {
  ...
  async componentDidMount() {
    ...
    if (this.state.checkedKey) {
      const keys = this.state.checkInList[this.state.checkedKey] ?
                Object.keys(this.state.checkInList[this.state.checkedKey]) : [];
      if (keys.includes('checkOut')) {
        this.setState({
          region: {
            latitude: this.state.checkInList[this.state.checkedKey].checkOut.location.latitude,
            longitude: this.state.checkInList[this.state.checkedKey].checkOut.location.longitude,
            latitudeDelta: 0.015,
            longitudeDelta: 0.015,
          },
          marker: {
            ...
            description: "[" + this.state.checkInList[this.state.checkedKey].checkOut.location.latitude
                        + ", " + this.state.checkInList[this.state.checkedKey].checkOut.location.longitude
                        + "]",
          },
        });
  ...
  async _writeDB(time) {
    set(ref(getDatabase(), 'checked/' + this.state.checkedKey + '/'), {
      user: getAuth().currentUser.email.split('@')[0],
      checkIn: this.state.checkInList[this.state.checkedKey].checkIn,
      checkOut: {
        datetime: time,
        location: {
  ...
  async checkOutPressed(time) {
    await this._writeDB(time)
    await this._readDB()
    LineNotify("ออก", getAuth().currentUser.email.split('@')[0],
  ...
  checkOutButton() {
    ...
    if (!this.state.checkedKey) {
      return (
        <Text style={[styles.checkedText, {color: '#FF5252'}]}>
          ยังไม่ได้เข้างาน
        </Text>
      )
    } else if (keys.includes('checkOut')) {
      return (
        <>
          ...
          <View style={styles.group}>
            ...
            <Text style={styles.checkedText}>
              { moment(this.state.checkInList[this.state.checkedKey].checkOut.datetime)
                .format('HH:mm:ss') }
            </Text>
          </View>
        </>
        ...
        <>
          ...
          <View style={styles.group}>
            <TouchableHighlight
              ...
              onPress={() => this.checkOutPressed(moment().format())}
            >
